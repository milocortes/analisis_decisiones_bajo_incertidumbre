...
# ╔═╡ 799e1afe-0f6e-11f0-3cef-6d7f31b8864a
begin
	# Load packages
...
# ╔═╡ 584ae4d6-24a6-4644-96b6-e4b4b475ba00
md"""
# SIR Epidemic Model

Scottish scientists W.O. Kermack and A.G. McKendrick developed the SIR model of disease infection in 1927. The model separates a population into three groups: susceptible (S), infected (I), and recovered (R), hence the name. The model uses three first-order differential equations to track infection in a population over time:

```math
\begin{aligned}
	\dfrac{dS}{dt} &= -\beta * \Big( \dfrac{SI}{N}\Big)\\
	\dfrac{dI}{dt} &= \beta * \Big( \dfrac{SI}{N}\Big) - \gamma I\\
	\dfrac{dR}{dt} &=  \gamma I
\end{aligned}
```

In the equation, $S$, $I$, and $R$ represent the number of people currently susceptible, infected, or recovered. It is assumed that the total population, $N = S +I + R$, is fixed. “Recovered” means recovered or died; we’ll attempt to separate the two outcomes later in the section.

We can interpret the parameters governing the model, $\beta$ and $\gamma$, as follows: a diseased person infects an average of $\beta$ people per day for an average of $\dfrac{1}{\gamma}$ days. Therefore, $\beta$ represents the disease transmission rate, and $\gamma$ is the disease recovery rate. The model assumes that after a person enters the $R$ group, they stay in the $R$ group because they have either died or cannot become infected again. This makes the SIR model useful when recovery implies an inability to become reinfected.

The average number of infections due to an infected individual is

```math
R_0 = \dfrac{\beta}{\gamma}
```

known as the *basic reproduction number*. The higher $R_0$ is, the more infectious the disease.

The next table shows the typical range of $R_0$ values for specific diseases, along with estimated $\beta$ and $\gamma$ values.

| Disease              | $R_0$        |   $\beta$ |   $\gamma$ |
|:---------------------|:----------|-------:|-------:|
| Measles              | 12–18     |   1.25 | 0.0833 |
| Pertussis            | 12–17     |   1.04 | 0.0714 |
| Diphtheria           | 6–7       |   0.54 | 0.0833 |
| Rubella              | 5–7       |   0.46 | 0.0714 |
| Mumps                | 4–7       |   0.34 | 0.0625 |
| COVID-19 (original)  | about 2.5 |   0.28 | 0.1111 |
| Influenza (seasonal) | 1.3–1.8   |   0.31 | 0.2    |

Let’s implement the SIR model and put it to the test before extending it to SIRD to track deaths due to disease. We’ll experiment with the set of diseases in the table by using the $\beta$ and $\gamma$ values that $I$ estimated from the mean $R_0$ and typical infectious period for each disease ($\gamma$). The values in the table are for pedagogical purposes only and should not be interpreted as medically meaningful.

"""

# ╔═╡ 956070ae-b52a-4453-96d7-43dd62643153
md"""
## Implementing and Testing the SIR Model
"""

# ╔═╡ 6a3f8223-00e7-4147-970f-c34219ec506f
begin 
	@mtkmodel FOL begin
    @parameters begin
...
# ╔═╡ 7614244f-e1c8-47b4-a654-b9907f13b0eb
begin
	days = 60.0
	h = 1
	prob_mtk = ODEProblem(fol, [], (0.0, days), [])
	sol_mtk = solve(prob_mtk,RK4(),dt=h,adaptive=false)
end

# ╔═╡ 85d2497a-f992-4058-9776-16c298725914
plot(sol_mtk, title = "SIR Model", xlabel = "Time",  ylabel = "Persons")

# ╔═╡ 8821e9a6-90f7-4943-a268-2cc65fc39465
md"""
## Implementing and Testing the SIRD Model

The SIRD Model retains $\beta$ and $\gamma$ but adds $\sigma$, the mortality rate per day. This introduces a fourth differential equation
...
to model $D(t)$, the number of deaths as a function of time. It also modifies the infection rate equation to add a new term

```math
\begin{aligned}
	\dfrac{dI}{dt} &= \beta \Big( \dfrac{SI}{N}\Big) - \gamma I - \mu I
\end{aligned}
```

to account for those who die from the disease.

Let’s run the model twice for our hypothetical small town of 1,200 people, where initially 1 percent are infected and the remaining 99 percent are susceptible.

The first run models a highly infectious and fatal disease: $\beta = 0.8$, $\gamma = 0.05$, and $\mu = 0.1$. The second run is for a disease similar to COVID-19 that’s less infectious and significantly less fatal. SIRD model $\sigma$ values are dificult to locate, so we’ll pick one for COVID-19 corresponding to a $\sigma = 0.004$ and use $\beta$ and $\gamma$ from the table.
"""
