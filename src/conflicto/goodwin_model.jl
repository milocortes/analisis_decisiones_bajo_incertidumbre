...
begin 
	...
	solution = solve(prob, RK4(), dt = 0.01)
...
```math
\begin{aligned}
	\dfrac{dx}{dt} &= (\alpha - \beta y(t)) x(t) = \alpha x(t) -\beta y(t)x(t)\\
	\dfrac{dy}{dt} &= (\delta x(t) - \gamma)y(t) = \delta x(t)y(t) - \gamma y(t)
\end{aligned}
```


where $x(t)$ y $y(t)$ denote the populations of prey and predator at time $t$, respectively, and $\alpha$, $\beta$, $\gamma$, $\delta$  are positive parameters.

We implement the Lotka-Volterra model and simulate it with parameters $\alpha = 1.5$, $\beta=1$, $\gamma = 3$ and $\delta=1$ and initial conditions $x(0) = 1$ and $y(1) = 1.$. The parameters $\alpha$ and $\gamma$ are the growth rate of the prey and the death rate of the predator, respectively.
...
* El nivel de producto determina el nivel de empleo $L$ via la productividad laboral $a$: 

```math
L=\frac{Y}{a}
```

* El nivel de empleo determina la tasa de empleo $\lambda$ (la proporción de $L$ con respecto a la población $N$): 

```math
\lambda = \frac{L}{N}
```

* La tasa de empleo determina el cambio en los salario reales $w$ vía la curva de Phillips: 

```math
\frac{dw}{dt} \frac{1}{w}=(-c + d \cdot \lambda)
```

* El nivel de las ganancias, $\Pi$, está dado por: 

```math
\Pi = Y- w \cdot L
```

* Las ganancias determinan la inversión $I$ (en este modelo, todas las ganancias son invertidas): 
```math
I = \Pi
```

La inversión menos la depreciación ($\gamma$) determina la tasa de cambio del stock de capital $K$, cerrando el modelo:
...
begin
	...
	@mtkbuild mtk_model = GoodwinModel()
...
manifest_format = "2.0"
project_hash = "3574dbf847ad8ad37720226d98073fc76bc8a14b"
